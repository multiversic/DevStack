...
export default function LoginPage() {
    ...
    const onSubmit = async (data: LoginInput) => {
        ...
        try {
            const res = await signIn("credentials", {
                redirect: false, // On gère manuellement la redirection pour l'UX (taux de rebond)
                email: data.email,
            ...
            if (res?.error) {
                ...
                router.refresh() // Force le rechargement de la layout root pour NextAuth
            }
        } catch (e) {
    ...
    return (
        <div className="min-h-screen flex items-center justify-center bg-background p-4 relative overflow-hidden">
            {/* Background subtil (Glassmorphism effect placeholder) */}
            <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[800px] h-[600px] bg-primary/10 rounded-full blur-3xl opacity-50 -z-10"></div>
